...
class Generator(nn.Module):
    def __init__(self, z_dim, num_classes, base_ch=64, img_size=64):
        ...
        # Determine number of blocks based on target image size
        if img_size == 32:
            ...
            self.blocks = nn.ModuleList([
                ...
                GenBlock(base_ch * 4, base_ch * 2, num_classes),
                GenBlock(base_ch * 2, base_ch * 1, num_classes),
                GenBlock(base_ch * 1, base_ch * 1, num_classes),
            ])
        elif img_size == 128:
            self.blocks = nn.ModuleList([
                GenBlock(base_ch * 8, base_ch * 4, num_classes),
            ...
            raise ValueError(f"Unsupported img_size: {img_size}. Supported: 32, 64, 128")
